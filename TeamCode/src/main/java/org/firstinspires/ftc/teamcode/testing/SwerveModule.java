...
public class SwerveModule {
    ...
    public SwerveModule(String name, DcMotorEx motor, CRServo servo, AnalogInput encoder,
                        double offset, boolean reversed, double[] pidVals) {
        this.name = name;
        this.motor = motor;
        this.servo = servo;
    ...
    /** Directly set motor power for driving / rotating */
    public void setPower(double power) {
        motor.setPower(reversed ? -power : power);
    }

    /** Rotate module to a target angle (simplified) */
    public void rotateTo(double angle) {
        // Here you can add servo rotation logic if needed
        // For now, we assume a CRServo controls wheel angle
        double pos = angle / 360.0;   // normalize 0..1
        pos = Math.max(0.0, Math.min(1.0, pos));
        servo.setPower(pos);           // adjust as needed for your servo
    }
}
